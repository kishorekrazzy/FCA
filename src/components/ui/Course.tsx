...
export function CourseCard({ course }: { course: Course }) {
 ...
 const status = !enrolled ? null : progress >= 100 ? 'done' : 'progress'
 return <Link to={`/academy/${course.slug}`} className="poster-card fx-spotlight" onMouseMove={onMove} style={{ '--course-color': course.color } as React.CSSProperties}>
  ...
  {status && <span className={`poster-status ${status}`} aria-hidden="true">{status === 'done' ? <Check size={13}/> : <Play size={11}/>}</span>}

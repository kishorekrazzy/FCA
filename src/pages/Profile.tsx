...
import { Award, BookOpen, Check, Clock, Medal, MessageSquare, UserPlus, Users, X } from 'lucide-react'
...
type ProfileUser = { displayName?: string; photoURL?: string; xp?: number; streak?: number; completed?: string[]; enrolled?: string[]; publicId?: string; equippedFlair?: string }
...
export function Profile() {
 ...
 return <main className="profile-page page">
  <Reveal><section className="profile-hero"><span className="profile-avatar-ring"><span className="profile-avatar">{user.photoURL ? <img src={user.photoURL} alt="" referrerPolicy="no-referrer"/> : (user.displayName ?? '?').slice(0, 2).toUpperCase()}</span></span><div><span className="kicker">Learner profile{user.publicId && <> · {user.publicId}</>}</span><h1>{user.displayName ?? 'FCA Learner'} {user.equippedFlair && <span className="profile-flair" title="Equipped flair">{user.equippedFlair}</span>}</h1><div className="profile-level-row"><span className="profile-level-pill">Level {levelFor(xp)}</span><span className="profile-hero-meta">{xp.toLocaleString()} <img className="mini-icon" src="/icon-xp.svg" alt=""/> IQ · {completed.length} lessons completed</span></div></div>
  ...
  <Reveal delay={80}><section className="profile-stats"><article><span className="profile-stat-badge iq"><img className="stat-icon" src="/icon-xp.svg" alt=""/></span><strong>{xp.toLocaleString()}</strong><span>total IQ</span></article><article><span className="profile-stat-badge streak"><img className="stat-icon" src="/icon-streak.svg" alt=""/></span><strong>{user.streak ?? 0}</strong><span>day streak</span></article><article><span className="profile-stat-badge cert"><Award/></span><strong>{certificates}</strong><span>certificates</span></article></section></Reveal>
  <Reveal delay={140}><section className="profile-section"><h2><span className="section-icon-chip premium"><Medal size={14}/></span> Achievement shelf</h2><AchievementGrid stats={{ completed, streak: user.streak ?? 0, xp, certificates }}/></section></Reveal>
  <Reveal delay={180}><section className="profile-section"><h2><span className="section-icon-chip brand"><Users size={14}/></span> Friends {friendIds.length > 0 && <span className="friend-count">{friendIds.length}</span>}</h2>{friendProfiles.length ? <div className="friends-grid">{friendProfiles.map((friend) => <Link to={`/profile/${friend.id}`} className="friend-chip" key={friend.id} title={friend.displayName}>{friend.photoURL ? <img src={friend.photoURL} alt="" referrerPolicy="no-referrer"/> : (friend.displayName ?? '?').slice(0, 2).toUpperCase()}</Link>)}</div> : <p className="admin-empty">No connections yet.</p>}</section></Reveal>
  <Reveal delay={220}><section className="profile-section"><h2><span className="section-icon-chip brand"><BookOpen size={14}/></span> Courses in progress</h2>{enrolledCourses.length ? <div className="continue-list">{enrolledCourses.map((course) => <article key={course.slug}><CourseArt course={course}/><div><span className="kicker">{course.category}</span><h3>{course.title}</h3><p>{progressFor(course)}% complete</p><ProgressBar value={progressFor(course)}/></div><Link className="button ghost" to={`/academy/${course.slug}`}>View course</Link></article>)}</div> : <p className="admin-empty">No courses started yet.</p>}</section></Reveal>
 </main>
}

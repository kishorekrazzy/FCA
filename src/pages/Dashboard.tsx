import { ArrowRight, Award, Check, Gift, Map, RotateCcw, Trophy } from 'lucide-react'
...
import { achievementRules, dateKey, levelFor, levelProgress, useAcademyStore, xpToNext, type ReviewQuality } from '../store/academy-store'
...
function WeeklyChallenges({ streak, activityLog, claimed, onClaim }: { streak: number; activityLog: Record<string, number>; claimed: string[]; onClaim: (id: string, xp: number) => void }) {
 ...
 return <section className="dash-challenges"><div className="section-heading"><div><span className="kicker"><Trophy size={12}/> Time-boxed</span><h2>Challenges & <em>events.</em></h2></div></div>
  <div className="challenges-grid">{active.map((challenge) => {
   ...
   return <article className={`challenge-card ${done ? 'done' : ''}`} key={challenge.id}>
    ...
    <div className="challenge-body"><h3>{challenge.title}</h3><p>{challenge.description}</p><ProgressBar value={Math.round((value / challenge.goalCount) * 100)}/><span className="challenge-meta">{value}/{challenge.goalCount} {challenge.goalType === 'streak' ? 'day streak' : 'lessons'} · +{challenge.xpReward} XP</span></div>
